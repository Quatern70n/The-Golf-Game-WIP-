...
from SaveManager import Saver
#в конце важные строки с коментариями
...
START_PAUSE = 0
FINISH_PAUSE = 0
PAUSE_TIME = FINISH_PAUSE - START_PAUSE
    ...
    def __init__(self, x, y, width, height, image, sound=None):
        ...
        if width > 100:
            if "value" not in image and "select" not in image and "used" not in image:
                ...
                self.background_button = pygame.transform.scale(self.background_button, (width + 20 if width < 140 else width + 50, height + 10))
                self.fon = True
...
class Menu:
    ...
    def play(self):
        ...
        #группа с level если меняете то надо менять и при нажатие клавиш arrow_right, arrow_left
    ...
    def customization(self):
        ...
        #я немного каряво сделал выбор цвета сдесь все цвета
...
class Play:
    def __init__(self, color, level, sound_eff):
        ...
        if self.window:
            ...
            self.screen = pygame.display.set_mode(size)
        print(random.choice(self.music_fon))
    ...
    def game(self):
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    running = False
                #здесь я сделал для тестов по нажатию пробела, конец игры all_time надо будет сохранять
                elif event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE:
                        time_finish = (time.perf_counter() -START_TIME) - PAUSE_TIME
                        all_time = str(datetime.datetime.fromtimestamp(time_finish).strftime('%M:%S'))
                        save_time = Saver('saves/test')
                        save_time.save('название уровня', {'time': all_time})# Я не нашел названия уровней
                        # Если уровень не был до этого сохранен, то лучше save_time.write('название уровня', {'time': all_time})
                        # Если хочешь сохранять только лучший результат, то save_time.filter_save('название уровня', {'time': all_time}) можешь 3-м аргументом другую функцию ставить чтобы по другому признаку выбирать
                        # Это работает со всеми сохранениями. Если хочешь больше данных сохранять, то в словарь который вторым аргументом добавь.
                        # save_time.read('название уровня') чтобы извлечь информацию. Можешь вторым аргументом название задать извлечь только один параметр
                        Finish(all_time, self.color, self.level, self.sound_eff)
                        
                    if event.key == pygame.K_ESCAPE:
                        START_PAUSE = time.perf_counter()
                        Pause(self.color, self.level, self.sound_eff)
...
class Finish:
    ...
    def fin(self):
        ...
        text_1 = font.render(self.time, True, (255, 221, 119, 1))

        font1 = pygame.font.Font("fonts/JejuHallasan-Regular.ttf", 48)
        #здесь лучшее время его надо загружать из сейов
        text_2 = font1.render("20:20", True, (255, 221, 119, 1))
        text_3 = font1.render("40", True, (255, 221, 119, 1))
...
""" секундомер сейчас ненужен я переделал
class Stopwatch:
    def __init__(self):
    ...
    def quit_pause(self):
        self.__time_exit = time.perf_counter()
        self.__stop += self.__time_exit - self.__time
        print(self.__stop)

    def get_elapsed_time(self):
        __elapsed_time = (time.perf_counter() - self.__start_time) - self.__stop
        print((time.perf_counter() - self.__start_time) - self.__stop, time.perf_counter())
        return str(datetime.datetime.fromtimestamp(__elapsed_time).strftime('%M:%S'))
"""
...
class Pause:
    ...
    def is_paused(self):
        ...
        clock = pygame.time.Clock()
        background = pygame.image.load("data/background_pause.png")
        background = pygame.transform.scale(background, (self.width, self.height))

        
        volume_zero = Button(744, 489, 106.663, 85.306, "data/value_zero.png", sound="music/button_click.mp3")
        volume_max = Button(906.68, 489, 106.663, 85.306, "data/value_plus.png", sound="music/button_click.mp3")
        volume_min = Button(1069.37, 489, 106.663, 85.306, "data/value_min.png", sound="music/button_click.mp3")
        button_again = Button(952, 759, 206, 105, "data/button_again.png", sound="music/button_click.mp3")
        button_continue = Button(857, 663, 206, 105, "data/button_continue.png", sound="music/button_click.mp3")
        button_menu = Button(762, 759, 206, 105, "data/button_menu.png", sound="music/button_click.mp3")
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        FINISH_PAUSE = time.perf_counter()
                        PAUSE_TIME += FINISH_PAUSE - START_PAUSE
                        running = False
                    
                elif event.type == pygame.USEREVENT and event.button == button_again:
                    START_TIME = 0
                    START_PAUSE = 0
                    FINISH_TIME = 0
                    FINISH_PAUSE = 0
                ...
                elif event.type == pygame.USEREVENT and event.button == button_continue:
                    FINISH_PAUSE = time.perf_counter()
                    PAUSE_TIME += FINISH_PAUSE - START_PAUSE
                    running = False
                    

                elif event.type == pygame.USEREVENT and event.button == button_menu:
                    Menu()
                ...
                for button in [volume_max, volume_min, volume_zero, button_again, button_continue, button_menu]:
                    button.event(event)

            for button in [volume_max, volume_min, volume_zero, button_again, button_menu, button_continue]:
                button.homing(pygame.mouse.get_pos())
                button.draw(self.screen)
